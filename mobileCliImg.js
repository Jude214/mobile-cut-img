...
var ClipImgUtile = (function () {
    ...
    /**
     * 添加style 到你head中的style标签中
     * @param str
     */
    function addStyle(str) {
        var styleTag = document.head.style;
        if (styleTag) {
            styleTag.sheet.insertRule(str);
        } else {
...
ClipImg.prototype = {
     ...
     */
    init: function () {
        this.$id.addEventListener('change', function () {
            this.main();
        }.bind(this));
        var style =  document.createElement('style');
        style.setAttribute('id','clipImg');
    ...
    //  */
    css: function () {
        return".ClipImgTap{position:fixed;height:100vh;width:100vw;display:-webkit-box;display:-ms-flexbox;display:flex;-webkit-box-pack:center;-ms-flex-pack:center;justify-content:center;-webkit-box-align:center;-ms-flex-align:center;align-items:center;left:0;top:0;z-index:2}.ClipImgTap .ClipDiv{position:absolute;left:50%;top:50%;-webkit-transform:translate(-50%,-50%);transform:translate(-50%,-50%);height:600px;width:600px;border:2px solid white;box-sizing:border-box;overflow:hidden;display:-webkit-box;display:-ms-flexbox;display:flex;-webkit-box-align:center;-ms-flex-align:center;align-items:center;-webkit-box-pack:center;-ms-flex-pack:center;justify-content:center}.ClipImgTap .backDiv{background-color:black;min-height:100vh;min-width:100vw;display:-webkit-box;display:-ms-flexbox;display:flex;-ms-flex-wrap:wrap;flex-wrap:wrap;-webkit-box-align:center;-ms-flex-align:center;align-items:center;-webkit-box-pack:center;-ms-flex-pack:center;justify-content:center}.ClipImgTap .backDiv img{-webkit-filter:blur(5px)}.ClipImgTap .CloseDiv{position:absolute;right:20px;background-color:black;z-index:21;height:40px;width:40px;top:40px}.ClipImgTap .cli_btns{position:absolute;left:0;bottom:0;width:100%;display:-webkit-box;display:-ms-flexbox;display:flex;height:146px}.ClipImgTap .cli_btns::before{content:' ';position:absolute;left:0;right:0;width:100%;height:100%;opacity:.6;background-color:#000}.ClipImgTap .cli_btns .cli_commit,.ClipImgTap .cli_btns .cli_cancle{-webkit-box-flex:1;-ms-flex:1;flex:1;color:white;line-height:146px;z-index:1;font-size:34px;padding:0 60px}.ClipImgTap .cli_btns .cli_commit{text-align:right}";
    },
    /**
     * 裁剪
     */
    ...
    replaceFileInput: function () {
        var $id = this.$id;
        var cloneHtl = $id.cloneNode();
        $id.parentNode.replaceChild(cloneHtl, $id);
        this.$id = document.getElementById(this.fileId);
        this.init();
    },
    /**
     * 关闭蒙版
     */
    close: function () {
        var removeEl = document.querySelector('.ClipImgTap');
    ...
    main: function () {
        var self = this,
            ...
            loadImg = new Image();
        self.imgType = f.type;
        if (format && self.format.indexOf(f.type.split('\/').pop()) == -1) {
            self.replaceFileInput();
            return dealFun({status: 3});
        }
        dealFun({status: 0});
        ...
        loadImg.onerror = function () {
            return dealFun({status: 6});
        };
        loadImg.src = url;
    },
    /**
     * 通过orientation 来重新计算裁剪的长宽以及反转角度
     * @param orientation
     * @param width
     * @param height
     * @param url
    ...
    showVess: function (url) {
        var self = this,
            ...
            targetDensitydpi = self.targetDensitydpi,
            utile = self.utile,
            createElement = utile.createElement,
            fragmente = document.createDocumentFragment(),
            dealFun = self.dealFun;
        dealFun({status: 1});
        ...
        var ClipDiv = createElement('div', {'class': 'ClipDiv'});
        var backDiv = createElement('div', {'class': 'backDiv'});
        var cancle = createElement('a', {'class': 'cli_cancle'}, '取消');
        var commit = createElement('a', {'class': 'cli_commit'}, '保存');
        var btns = createElement('div', {'class': 'cli_btns'});
        self.addClose(cancle);
        self.addCut(commit);
        btns.appendChild(cancle);
        btns.appendChild(commit);
        ClipDiv.appendChild(img);
        backDiv.appendChild(img2);
        ClipImgTap.appendChild(backDiv);
        ClipImgTap.appendChild(ClipDiv);
        ClipImgTap.appendChild(btns);
        fragmente.appendChild(ClipImgTap);
        self.addMove(ClipDiv);
